	...
	var AudDRecorder = function(){
		...
		const REC = (() => {
			...
			// A state variable to prevent multiple recordings from starting simultaneously.
                        ...
                        // Map original media elements to their hidden CORS clones
                        const corsClones = new WeakMap();
			...
			const REC = {
				...
				/**
					...
					m_elm.currentTime = currentTime;
                                ...
                                /**
                                 * Create a low-volume hidden clone for CORS sources.
                                 * @param {HTMLMediaElement} elem
                                 * @returns {HTMLMediaElement} The cloned element
                                 */
                                createCorsClone(elem) {
                                        if (corsClones.has(elem)) {
                                                return corsClones.get(elem);
                                        }
                                        const clone = document.createElement('audio');
                                        clone.style.display = 'none';
                                        clone.src = elem.currentSrc;
                                        ...
                                        const cleanup = () => {
                                                clone.pause();
                                                clone.remove();
                                                corsClones.delete(elem);
                                                elem.removeEventListener('pause', cleanup);
                                                elem.removeEventListener('ended', cleanup);
                                                window.removeEventListener('pagehide', cleanup);
                                        };
                                        elem.addEventListener('pause', cleanup);
                                        elem.addEventListener('ended', cleanup);
                                        window.addEventListener('pagehide', cleanup, { once: true });
                                        clone.addEventListener('canplay', () => clone.play().catch(() => {}), { once: true });
                                        corsClones.set(elem, clone);
                                        return clone;
                                },
					...
					try {
						...
						// --- Main Element Processing Loop ---
                                ...
                                if (await REC.isCorsSource(m_elm)) {
                                    m_elm = REC.createCorsClone(m_elm);
                                }
