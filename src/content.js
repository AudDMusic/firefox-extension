			...
			const REC = {
				...
				async start(rec_time_ms = 2000, mime = 'audio/webm') {
					...
					try {
                                                ...
                                                const elementsForRecording = [];
                                                for (const elm of mediaElements) {
                                                        if (await REC.isCorsSource(elm)) {
                                                                try {
                                                                        const clone = await REC.createOffscreenClone(elm);
                                                                        elementsForRecording.push(clone);
                                                                } catch (e) {
                                                ...
                                                for (const m_elm of elementsForRecording) {
							try {
